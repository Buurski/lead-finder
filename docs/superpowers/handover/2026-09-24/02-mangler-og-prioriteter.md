...
2. Rigtig opvarmning: nyt domæne-send → maks ~20/dag i starten, 22–45 s pause (findes), ingen billeder/tracking-pixels, og list-unsubscribe-header (mangler — tjek). **Afgjort 25/9 (bølge 2):** loft = atomisk dagsbudget 20/konto delt af kold-kø og preview-send. List-Unsubscribe **fravalgt** efter A/B-testmail: med header → Gmail Promotions, uden → Primær (SPF/DKIM/DMARC pass i begge); Gmails krav gælder først >5000/dag. OBS (Sol R2): kolde kladder fra /approve har i dag INGEN afmeld-linje i selve teksten (kun den gamle skabelon-sti havde den) — om der skal tilføjes én, er Lucas' beslutning (ændrer mailteksten). Se plan fase 5, bølge 2.
