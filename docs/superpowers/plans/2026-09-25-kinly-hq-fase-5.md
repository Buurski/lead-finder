...
7c. Google Kalender: GJORT 25/9 nat. Workspace blokerer redigerings-deling UD (kun free/busy) → `scripts/gcal-setup.mjs` lod service-accounten eje "Kinly HQ – Lucas" og dele den IND til lucas@kinly.dk (læser). `HQ_GCAL_LUCAS` sat i Vercel prod. Lucas: tilføj kalenderen fra Googles mail + sæt notifikationer på den. Calendar API var slået til; Search Console API slået til 25/9 (projekt leads-494721). GSC: SA tilføjet som Restricted på sc-domain:ikastautoservice.dk.
7d. Search Console: Ikast er gjort (se 7c); nye kunder: tilføj SA som Restricted. Kør `/api/cron/gsc-snapshot` manuelt én gang (Bearer CRON_SECRET) → kort "Bliver fundet på Google" på profilen; 'ingen adgang' pr. kunde står i health-noten. Opdateringer kommer fra 2. måling (mandagen efter).
