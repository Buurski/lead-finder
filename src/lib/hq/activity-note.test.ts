...
test("retter en egen note og markerer den redigeret", async () => {
  ...
  const after = await updateNote(db, n.id, companyId, "Rettet tekst");
...
test("sletter en egen note", async () => {
  ...
  await deleteNote(db, n.id, companyId);
...
test("afviser ikke-note-typer", async () => {
  ...
  await assert.rejects(updateNote(db, a.id, companyId, "ny tekst"), NoteError);
  await assert.rejects(deleteNote(db, a.id, companyId), NoteError);
});
...
test("afviser system-/agent-noter", async () => {
  ...
  await assert.rejects(updateNote(db, n.id, companyId, "ny tekst"), NoteError);
  await assert.rejects(deleteNote(db, n.id, companyId), NoteError);
});
...
test("tom tekst og forkert virksomhed afvises", async () => {
  ...
  await assert.rejects(updateNote(db, n.id, companyId, "  "), NoteError);
  await assert.rejects(updateNote(db, n.id, "00000000-0000-0000-0000-000000000000", "ny tekst"), NoteError);
});
