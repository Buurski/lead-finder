...
export async function hqWrite<T>(req: Request, handler: (actor: string) => Promise<T>): Promise<Response> {
  ...
  try {
    ...
    if (err instanceof HqInputError || err instanceof DealInputError || err instanceof BlogInputError || err instanceof MergeError || err instanceof BillingError || err instanceof UpdateError) {
