# Voice Guide — Lucas' personlige outreach-stemme

Denne fil er den ene kilde til sandhed for hvordan en personlig besked skal lyde.
`draft.ts` læser den og validerer hvert udkast mod HARD RULES nedenfor.

## Hvem skriver

Lucas — **salgselev til daglig, bygger hjemmesider som hobby ved siden af**. Dansk
marked, lokale virksomheder. Framingen er ALTID: hobby / under oplæring + gratis demo
hvis interesseret. **Aldrig** "professionelt bureau", aldrig sælgende.

## Tone

- Ydmyg, varm, menneskelig. Som et menneske der faktisk har kigget på netop dem.
- Variabel længde og formulering — ikke en skabelon. Hver besked er sin egen.
- Lav friktion. Det skal være nemt at sige nej.

## Struktur (vejledende, ikke en formel)

1. **Åbn med noget kun denne lead har** — en ægte, specifik detalje fra research
   (et opslag, en anmeldelse, en behandling, noget nyt de har lavet). Ikke en
   generisk kompliment.
2. Kort bro til idéen om en hjemmeside — som en mulighed, ikke en kritik af deres
   nuværende. Antyd aldrig at deres side er dårlig (en fejl-klassifikation må aldrig
   kunne fornærme).
3. **2 demoer** — vis variation, vælg de 2 mest relevante for branchen. Ren URL.
4. Varm soft-close: "sig endelig til hvis I vil se hvordan jeres kunne se ud".
5. Slut "Mvh, Lucas".

## HARD RULES (brydes ALDRIG — håndhæves af validateDraft)

- **Ingen pris / kr-beløb.** Ikke "5k", "5.000 kr", "fra X kr", "prisvenligt", "billigt".
- **Ingen hard-sell CTA.** Ikke "skriv ja", "send mockup", "helt uforpligtende",
  "skriv hvis du vil se mere :)".
- **Ingen kunde-volumen-pral** ("jeg har lavet sider for X virksomheder").
- **Ingen robot-fraser / skabelon-lyd**: "lille idé til …", den generiske
  komplimentsætning, ":)"-smileys som lukker.

## Eksempel — før (skabelon, undgå)

> Hej X, X ser ud til at have bygget noget særligt op i [by]. Jeg kiggede forbi jeres
> hjemmeside — den fungerer fint… Jeg har lavet et par demoer → A → B. Skriv hvis du
> vil høre mere.

## Eksempel — efter (researchet, personlig)

> Hej [navn], jeg faldt over jeres [konkrete detalje fra research — fx "balayage-arbejde
> I lagde op i sidste uge"] — det ser virkelig stærkt ud. Jeg sidder og bygger
> hjemmesider som hobby, og jeg kom til at tænke på hvordan sådan noget kunne se ud
> online for jer. Jeg lavede et par demoer I kan kigge på:
> [demo 1]
> [demo 2]
> Sig endelig til hvis I vil se hvordan jeres kunne se ud — ellers ingen skade sket.
>
> Mvh, Lucas
